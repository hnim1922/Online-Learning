        ...
        <main>
            <div class="sider-course-list">
                ...
                <c:forEach var="featuredSubject" items="${featuredSubjectList}">
                    <div class="featured-subject-card" onclick="location.href = 'CourseDetails?subjectID=${featuredSubject.subjectID}';">
            ...
            <div class="course-details-container" style="width: 100%;">
                <div class="subject-thumbnail">
                    <img src="${requestScope.subjectThumbnail}" alt="subject-thumbnail"/>
                </div>
                <h1 style="margin-left: 20px; margin-bottom: 20px;  font-size: 3em;"><c:out value="${requestScope.subjectTitle}"/></h1>
                <div class="subject-categories-card" style="margin-left: 20px;">${requestScope.subjectCategory}</div>
                <p class="subject-info">${requestScope.subjectInfo}<p>
                <div class="price-package">
                    <h3 style="text-decoration: line-through;">$${requestScope.subjectPricePackage.listPrice}</h3>
                    <h2 class="detail-sale-price">$${requestScope.subjectPricePackage.salePrice}</h2>
                    <a class="login-btn" style="position: absolute; right: 0px; bottom: 20px;" <c:if test="${!empty sessionScope.USER}"> onclick="openCourseRegister()" </c:if> <c:if test="${empty sessionScope.USER}">onclick="openSignupForm()"</c:if> >Start Course</a>
                </div>
            </div>
        </main>
