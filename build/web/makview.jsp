<%-- 
    Document   : makview
    Created on : Jun 4, 2021, 2:24:08 PM
    Author     : Admin
--%>
    ...
    <body>
        ...
        <div class="topbar-right">
            <c:if test="${!empty requestScope.INVALID}">
                <button class="login-btn" id="profile" onfocus="autoClickProfile()" autofocus
                        onclick="document.getElementById('profile-form').style.display = 'block';
                                document.getElementById('change').focus()" >User Profile</button> <br>
            </c:if>
            <c:if test="${empty requestScope.INVALID}">
                <button class="login-btn" id="profile"
                        onclick="document.getElementById('profile-form').style.display = 'block'" >User Profile</button> 
            </c:if>
            <c:if test="${empty requestScope.INVALID}">
                <a class="sliderList" href="SliderList?page=1"> 
                    <div>    Slider List </div>
                </a>
                <a class="sliderList" href="PostList?index=1"> 
                    <div>    Post List </div>
                </a>
            </c:if>          
            ...
            <div id="change-form" class="modal">
                <form action="ChangeMak" method="POST" class="modal-content animate">
                    <table>
                        <tr>
