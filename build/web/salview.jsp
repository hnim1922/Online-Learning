<%-- 
    Document   : salview
    Created on : Jun 4, 2021, 2:23:30 PM
    Author     : Admin
--%>
    ...
    <body>
        ...
        <div class="topbar-right">
            <c:if test="${!empty requestScope.INVALID}">
                <button class="login-btn" id="profile" onfocus="autoClickProfile()" autofocus onclick="document.getElementById('profile-form').style.display = 'block';
                                document.getElementById('change').focus()">User Profile</button>
            </c:if>
            <c:if test="${empty requestScope.INVALID}">
                <button class="login-btn" onclick="document.getElementById('profile-form').style.display = 'block'">User Profile</button>
            </c:if>
            <button class="login-btn" onclick="location.href = 'RegistrationList'">Registration List</button>
            <!-- profile box -->
                    ...
                    <form method="POST">
                        ...
                        <table border="1">
                            ...
                            <tr>
                                ...
                                <td>
                                    <c:if test="${!empty requestScope.INVALID}">
                                        <button form="x" class="login-btn" id="change" onfocus="autoClickChange()" autofocus onclick="document.getElementById('change-form').style.display = 'block'
                                                        document.getElementById('profile-form').style.display = 'none'">Change Info</button>
                                    </c:if>
                                    <c:if test="${empty requestScope.INVALID}">
                                        <button form="x" class="login-btn" onclick="document.getElementById('change-form').style.display = 'block'
                                                        document.getElementById('profile-form').style.display = 'none'">Change Info</button>
                                    </c:if>
                                </td>
                            </tr>
            ...
            <div id="change-form" class="modal">
                <form action="ChangeSal" method="POST" class="modal-content animate">
                    <table>
                        <tr>
            ...
            <div id="action-box" class="modal">
                <div class="modal-content animate">
                    <h1 class="modal-content animate">ACTIONS</h1>
                    <table>
                        <tr>
