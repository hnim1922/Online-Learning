                                ...
                                <form method="POST">
                                    ...
                                    <table>
                                        ...
                                        <tr>
                                            ...
                                            <td id="button">
                                                <c:if test="${!empty requestScope.INVALID}">
                                                    <button form="x" class="login-btn" id="change"
                                                            onfocus="autoClickChange()" autofocus id="button"
                                                            onclick="document.getElementById('change-form').style.display = 'block'
                                                                    document.getElementById('profile-form').style.display = 'none'">Change Info</button>
                                                </c:if>
                                                <c:if test="${empty requestScope.INVALID}">
                            ...
                            <div class="sider-course-list">
                                ...
                                <c:forEach var="featuredSubject" items="${featuredSubjectList}">
                                    <div class="featured-subject-card"
                                         onclick="location.href = 'CourseDetails?subjectID=${featuredSubject.subjectID}'">
                                        <img src="${featuredSubject.thumbnail}" alt="alt"
                                             style="width: 90px;height: inherit; border-top-left-radius: inherit; border-bottom-left-radius: inherit;" />
                                ...
                                <div class="course-list-container" style="width: 100%;">
                                    <div class="course-list">
                                        <c:forEach var="course" items="${paidCourseList}">
                                            <div class="course-card">
                                                <img src="${course.subject.thumbnail}" alt="course-thumbnail" class="course-thumbnail" />
                                                <div class="course-information">
                                                    <h3><b>${course.subject.name}</b></h3>

                                                    <div class="subject-categories-card">${course.subject.categoryName}</div>

                                                    <p>${course.subject.information}</p>
                                                </div>
                                                <c:if test="${course.isStatus() == 0}">
                                                    <a href="LessonView?subjectID=${course.subject.subjectID}&lessonID=0&courseID=${course.courseID}" class="login-btn"
                                                       style="position: absolute; right: 0px; bottom: 20px;">Start Course</a>
                                                </c:if>
                                                <c:if test="${course.isStatus() != 0}">
                                                    <div class="progress-bar">
                                                        <div class="progress-complete" id="prgess-complete" style="width: ${course.isStatus() * 100}%"><fmt:formatNumber value="${course.isStatus() * 100}" minFractionDigits="0" maxFractionDigits="0"/>%</div>
                                                        <a href="LessonView?subjectID=${course.subject.subjectID}&lessonID=0&courseID=${course.courseID}" class="login-btn"
                                                           style="position: absolute; right: 0px; bottom: 20px;">Start Course</a>
                                                    </div>
                                                </c:if>
                                    ...
                                    <div class="page-route">
                                        <c:forEach var="i" begin="1" end="${Math.ceil(allSubjectList.size()/3)}">
                                            <a href="MyCourse?page=${i}" <c:if test="${param.page == i}">class="on-page"</c:if>
                                               >${i}</a>
                                        </c:forEach>
                                    </div>
                                </div>
